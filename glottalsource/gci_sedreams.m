...
function [gci, MeanBasedSignal, res] = gci_sedreams(wave, fs, f0mean, ...
    ...
    [~,PotMaxis]=findpeaks(MeanBasedSignal + abs(MeanBasedSignal));
    [~,PotMinis]=findpeaks(-MeanBasedSignal + abs(MeanBasedSignal));
    ...
    for k=1:length(Posis)
        ...
        [~,pos]=min(Dists);
        interv=abs(Maxis(pos)-Minis(pos));
